...
<template>
  ...
  >
    ...
    <div
      class="flex w-full max-w-md justify-around px-4 md:h-full md:max-w-none md:flex-col md:items-start md:justify-center md:gap-4 md:px-0"
    >
      <button
        v-for="tab in tabs"
        :key="tab.path"
        type="button"
        class="flex min-w-16 cursor-pointer flex-col items-center gap-1 rounded-xl px-2 py-1.5 text-[10px] font-medium transition-all duration-200 md:w-full md:min-w-0 md:flex-row md:gap-1.5 md:gap-2 md:py-2 md:text-sm"
