...
from src.support_bot.bot.utils.exceptions import CreateForumTopicException, NotEnoughRightsException
...
async def not_enough_rights_error(event: ErrorEvent, manager: Manager) -> None:
    ...
    for admin_id in manager.config.bot.DEV_IDS:
        await manager.bot.send_message(admin_id, NotEnoughRightsException.message)
...
async def create_forum_topic_error(event: ErrorEvent, manager: Manager) -> None:
    ...
    for admin_id in manager.config.bot.DEV_IDS:
        await manager.bot.send_message(admin_id, CreateForumTopicException.message)
...
async def telegram_api_error(event: ErrorEvent, manager: Manager) -> None:
    ...
    for admin_id in manager.config.bot.DEV_IDS:
        message = await manager.bot.send_document(admin_id, document, caption=caption)
