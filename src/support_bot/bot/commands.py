...
from src.support_bot.bot.utils.texts import SUPPORTED_LANGUAGES
from src.support_bot.config import Config
...
async def setup(bot: Bot, config: Config) -> None:
    ...
    group_commands = {
        "en": [
            ...
            BotCommand(command="open", description="Open ticket"),
        ],
        "ru": [
            ...
            BotCommand(command="open", description="Открыть тикет"),
        ]
    }
    ...
    for admin_id in config.bot.DEV_IDS:
        try:
            ...
            raise ValueError(f"Chat with ADMIN_ID {admin_id} not found.")
